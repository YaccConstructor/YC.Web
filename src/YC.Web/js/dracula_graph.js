...
addEdge:function(source,target,style){
	...
	var edge={source:s,target:t,style:style,weight:style&&style.weight||1};
	s.edges.push(edge);
	this.edges.push(edge);
	if(!style||!style.directed){
		var backedge={source:t,target:s,style:style,weight:style&&style.weight||1,backedge:edge};
		this.edges.push(backedge);t.edges.push(backedge);
	}
},
...
Graph.Renderer.Raphael=function(element,graph,width,height){
	this.width=width||400;this.height=height||400;var selfRef=this;
	this.r=Raphael(element,this.width,this.height);
	this.radius=40;
	this.graph=graph;
	this.mouse_in=false;
	if(!this.graph.render){
		this.graph.render=function(){return;}
	}
	this.isDrag=false;
...
document.onmouseup=function(){
	selfRef.isDrag&&selfRef.isDrag.set.animate({"fill-opacity":.6},500);selfRef.isDrag=false;};
};
...
draw:function(){
	...
	for(i in this.graph.nodes){this.drawNode(this.graph.nodes[i]);}
...
drawNode:function(node){
	...
	var shape;
	if(node.render){shape=node.render(this.r,node);}
	else if(node.shape){}
	else{
		var color=Raphael.getColor();
		shape=this.r.set().push(this.r.ellipse(point[0],point[1],30,20).attr({fill:color,stroke:color,"stroke-width":2})).push(this.r.text(point[0],point[1]+ 30,node.label||node.id));
	}
...
Graph.Layout.Spring.prototype={
	layout:function(){
		...
		this.layoutCalcBounds();},layoutPrepare:function(){
...
layoutAttractive:function(edge){
	...
	var dy=node2.layoutPosY- node1.layoutPosY;
	var d2=dx*dx+ dy*dy;if(d2<0.01){
		dx=0.1*Math.random()+ 0.1;dy=0.1*Math.random()+ 0.1;
		var d2=dx*dx+ dy*dy;
	}
	var d=Math.sqrt(d2);
	if(d>this.maxRepulsiveForceDistance){
...
Raphael.el.tooltip=function(tp){
	...
	function(event){this.tp.hide();this.unmousemove();});return this;};